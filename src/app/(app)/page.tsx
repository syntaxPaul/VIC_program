...
import { formatZAR, formatZARCompact, formatDate, enumLabel } from "@/lib/format";
              ...
              <tbody>
                {funds.map((f) => (
                  <tr key={f.id} className="hover:bg-sand-50 dark:hover:bg-sand-800/40">
                    ...
                    <td className="tnum border-b px-4 py-2.5 text-right text-[var(--text-muted)]">{formatZAR(f.opening, { decimals: false })}</td>
                    <td className="tnum border-b px-4 py-2.5 text-right text-success">{formatZAR(f.income, { decimals: false })}</td>
                    <td className="tnum border-b px-4 py-2.5 text-right text-danger">{formatZAR(f.expense, { decimals: false })}</td>
                    <td className="tnum border-b px-4 py-2.5 text-right font-semibold">{formatZAR(f.closing, { decimals: false })}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-sand-50 font-semibold dark:bg-sand-800/40">
                  ...
                  <td className="tnum px-4 py-2.5 text-right">{formatZAR(funds.reduce((s, f) => s + f.opening, 0), { decimals: false })}</td>
                  <td className="tnum px-4 py-2.5 text-right">{formatZAR(funds.reduce((s, f) => s + f.income, 0), { decimals: false })}</td>
                  <td className="tnum px-4 py-2.5 text-right">{formatZAR(funds.reduce((s, f) => s + f.expense, 0), { decimals: false })}</td>
                  <td className="tnum px-4 py-2.5 text-right">{formatZAR(totalFundBalance, { decimals: false })}</td>
                </tr>
              </tfoot>
            </table>
          </div>
